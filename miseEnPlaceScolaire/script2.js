...
function cleanThat() {
    ...
    sectionContainer.classList.remove('modify');
}
...
// Updated Function for getting student data from localStorage based on name
function getStudentData(name) {
    // Get saved data from localStorage
    let saved_data = localStorage.getItem('formData');
    let data = saved_data ? JSON.parse(saved_data) : {};

    // Loop through all classes and students to find the matching student
    for (let classe in data) {
        let students = data[classe][name];
        console.log(student)
        for (let studentName in students) {
            if (studentName === name) {
                return { name: studentName, class: classe, ...students[studentName][0] };
            }
        }
    }
...
// Function for updating student data using localStorage
...
_btnModify.addEventListener('click', function() {
  cleanThat();
  sectionContainer.classList.add('modify');

  titleSection.innerHTML = '<span class="firstLetter">M</span>odifier les information<span class="lastLetter">s</span>';
  
  // Generate a search field and a search button
  const searchField = document.createElement('input');
  searchField.setAttribute('id', 'searchField');
  searchField.setAttribute('placeholder', 'Enter student name...');
  ...
  searchBtn.addEventListener('click', function() {
    ...
    const studentData = getStudentData(searchTerm);
    
    if (studentData) {
      displayStudentInfo(studentData, searchTerm);
    } else {
...
// Function to display student's existing information and allow modification
function displayStudentInfo(studentData, studentName, studentClass) {
...
function deleteStudentData(studentName, studentClass) {
    // Récupérer les données existantes
    const savedData = localStorage.getItem('formData');
    const data = savedData ? JSON.parse(savedData) : {};
  
    // Vérifier si l'élève et la classe existent
    if (data[studentClass] && data[studentClass][studentName]) {
      // Supprimer l'élève
      delete data[studentClass][studentName];
      
      // Si la classe devient vide, la supprimer aussi
      if (Object.keys(data[studentClass]).length === 0) {
        delete data[studentClass];
      }
      ...
      alert("L'élève n'a pas été trouvé.");
    }
  }
