...
btnCtc.addEventListener('click', function() {
    ...
    // Creating the divs for colorized text
    var divContainer = document.createElement('div');
    divContainer.style.position = "relative";
    myTextArea = document.createElement('div');
    myTextArea.setAttribute('id', "textarea");
    myTextArea.setAttribute('contenteditable', 'true');
    myColoredText = document.createElement('div');
    myColoredText.setAttribute('id', "coloredText");
    divContainer.appendChild(myColoredText);
    divContainer.appendChild(myTextArea);
    myForm.appendChild(divContainer);
    ...
    // Now declare colorizeText
    function colorizeText() {
        var inputDiv = document.getElementById('textarea');
        var outputDiv = document.getElementById('coloredText');

        var text = inputDiv.innerText;
        var resultHTML = "";

        for (let i = 0; i < text.length; i++) {
            let char = text.charAt(i);
            if ("aeiou".includes(char.toLowerCase())) {
                resultHTML += "<span class='vowel'>" + char + "</span>";
            } else if (/[a-z]/i.test(char)) {
                resultHTML += "<span class='consonant'>" + char + "</span>";
            } else {
                resultHTML += char;
            }
        }
    ...
    // Now we can add event listener for textarea
    myTextArea.addEventListener('input', colorizeText());
})
